...
python -m pip install pytest pytest-cov pytest-github-actions-annotate-failures jupyter numpy matplotlib pandas seaborn scikit-learn
if errorlevel 1 (
...
:: 3) Конвертировать ноутбук в python-скрипт
echo [96m3. Converting Jupyter notebook to Python script...[0m
jupyter nbconvert --to python --output exercises_dirty exercises.ipynb
if errorlevel 1 (
    echo [91mnbconvert failed[0m
    if defined VIRTUAL_ENV call deactivate 2>nul
    popd & endlocal & exit /b 1
)

:: 4) Почистить скрипт (убрать комменты/пустые/вызовы get_ipython и input)
echo [96m4. Cleaning up the converted script...[0m
type exercises_dirty.py | findstr /v /r /c:"^\s*#" /c:"^\s*$" /c:"get_ipython()" /c:"input(" > exercises.py
...
:: 7) Финальная зачистка мусора (кроме .venv)
