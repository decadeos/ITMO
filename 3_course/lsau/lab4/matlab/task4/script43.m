...
plot(t, 0.25*t.^2, ':', 'LineWidth', 2.5, 'Color', [0.3, 0.3, 0.3]);
for i = 1:length(kVal)
    k = kVal(i);
    assignin('base', 'k', k);
    out = sim('model41');
...
legendStrings = [{'g_{3} = 0.25t^2'}, legendStrings]; 
...
xlim([-0.1, 60.3]); ylim([-10, 900.5]);
grid on; grid minor; box on; grid on;
grid on; grid minor; box on;

set(findall(gcf, '-property', 'FontName'), 'FontName', 'DejaVu Math TeX Gyre');
exportgraphics(gcf, '../../images/task4/y3.png', 'Resolution', 300);
...
ax.GridColor = [0.9, 0.9, 0.9]; ax.GridAlpha = 0.4;
ax.MinorGridColor = [0.95, 0.95, 0.95]; ax.MinorGridAlpha = 0.2;
xlim([-0.1, 60.1]); ylim([-3, 200.5]);
grid on; grid minor; box on; grid on;
grid on; grid minor; box on;

set(findall(gcf, '-property', 'FontName'), 'FontName', 'DejaVu Math TeX Gyre');
exportgraphics(gcf, '../../images/task4/e3.png', 'Resolution', 300);