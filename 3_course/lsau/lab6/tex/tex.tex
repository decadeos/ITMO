...
\begin{document}
	...
	При \(k = \dfrac{1}{5}\) линия годографа не обходит точку \((-1; 0)\), что говорит о том что количество неустойчивых полюсов замкнутой и разомкнутой систем одинаково. Поскольку разомкнутая система устойчива, то и замкнутая система будет устойчивой.\\[4pt] 
	...
	При \(k = 1\) и \(k = \dfrac{3}{2}\) линия годографа обходит точку \((-1; 0)\) по часовой стрелке, что говорит о неустойчивости замкнутой системы.\\[1.2pt] 
	...
	\[a_3 = 10 \qquad a_2 = 10k - 12 \qquad a_1 = 9k -1 \qquad a_0 = 4-k \]	
	...
	По критерию Гурвица я получила, что замкнутая система находится на границе
	устойчивости при
	...
	Рассматривая систему при \(k = 1\), запас устойчивости по амплитуде
	определяется как отношение критического коэффициента усиления к текущему:
	
	\[
	A_z = \frac{k_\text{кр}}{k} = \frac{1.42}{1} = 1.42.
	\]
	
	Это означает, что коэффициент усиления можно увеличить примерно в 1.42 раза,
	прежде чем система окажется на границе устойчивости.
	...
	\Longrightarrow \quad
	\omega_{cp}^2 =
	\frac{172.6 + \sqrt{13196.2}}{72}
	\Longrightarrow \quad
	\omega_{cp} \approx 2.0
	\]
	
	Теперь посчитаю в матлабе запас устойчивости по фазе и критическое \(\tau\) по формулам:. 
	
	\[\varphi_z = \pi + \varphi(\omega_{cp}) \qquad \tau_{kr} = \frac{\varphi_z}{\omega_{cp}}\]
	
	Полученные значения:
	
	\[\tau_{kr} \approx 1.2728 \qquad \varphi_z = 2.5456 \]
	...
	\begin{figure}[H]
		...
		\caption*{Рисунки 55-57: Годографы Найквиста при различных \(\tau\)}
	\end{figure}	
	
	Из построенных годографов видно, что с увеличением \(\tau\) годограф 
	сильнее закручивается. При \(\tau = \tau_{kr}\) он проходит через точку \((-1;0)\), что соответствует 
	границе устойчивости. При \(\tau < \tau_{kr}\) точка не охватывается и система устойчива, 
	а при \(\tau > \tau_{kr}\) происходит охват критической точки, значит замкнутая система будет неустойчивой.
	
	\subsubsection{Переходные характеристики}
	
	Выберу значения коэффициента запаздывания для каждой из областей значений, соответствующих как устойчивой системе, так и неустойчивой:
	
	\[\tau_1 = 1 \qquad \tau_2 = 2 \qquad \]
	...
	Для первого объекта:
	\[
	\omega_{cp} \approx 6.86 \quad \varphi_z \approx 2.27 \quad \tau_{cr} \approx 0.33
	\]
	
	Для второго объекта:
	\[
	\omega_{cp} \approx 2.0 \quad \varphi_z \approx 2.55 \quad \tau_{cr} \approx 1.27
	\]
