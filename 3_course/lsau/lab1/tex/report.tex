...
\begin{document}
	...
	\section{Задание. Многоканальная система в форме вход-состояние-выход}
	
	В этом задании мне нужно:
	
	\begin{itemize}
		\item Взять матрицы \(A\), \(B\) и \(C\) из и рассмотреть систему в форме:
		\[
		\dot{x}(t) = A x(t) + B u(t), \quad y(t) = C x(t)
		\]
		
		\item Построить структурную схему системы с использованием блоков элементарных операций (сумматоры, интеграторы, усилители), с явным обозначением каналов для каждой компоненты вектора состояния \(x(t)\).
		
		\item Смоделировать поведение системы при входных воздействиях \(u_1(t) = 1(t)\), \(u_2(t) = \sin(t)\) и нулевых начальных условиях \(x(0) = 0\).
		
		\item Представить в отчёте: математическую модель системы, структурную схему системы, графики входов \(u(t)\) и выходов \(y(t)\).
	\end{itemize}
	
	\subsection{Математическая модель системы}
	
	Рассмотрю линейную многоканальную систему:
	
	\[
	\dot{x}(t) = A x(t) + B u(t), \quad y(t) = C x(t)
	\]
	
	Соглавно моему варианту возьму матрицы системы:
	...
	\[
	\begin{cases}
		\dot{x}_1(t) = -9x_2(t) + 9 + 5\sin(t) \\
		\dot{x}_2(t) = x_1(t) - 3x_2(t) + 2 + 11\sin(t) \\
		y_1(t) = 5x_1(t) + 6x_2(t) \\
		y_2(t) = 3x_1(t) + 8x_2(t)
	\end{cases}
	\]
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\textwidth]{../images/task4/model.png}
		\caption{Структурная схема многоканальной системы в форме В-С-В}
	\end{figure}
	
	Схема наглядно показывает, как связаны между собой входы, состояния и выходы системы.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task4/y.png}
		\caption{График выходов \(y_1(t)\) и \(y_2(t)\)}
	\end{figure}
	
	\textbf{Итог: } 
	В этом задании я рассмотрела линейную многоканальную систему, задала матрицы \(A\), \(B\), \(C\) и построила её структурную схему. Смоделировала поведение системы при заданных входах и нулевых начальных условиях.
	
	
	\newpage
	\section{Вывод}
	
	В этой лабораторной работе я изучила моделирование линейных динамических систем как через одно уравнение высокого порядка, так и через систему дифференциальных уравнений первого порядка. На основе заданных матриц я построила систему в пространстве состояний и исследовала её поведение.
	
	С помощью симулинка я провела численное моделирование, получила графики и проанализировала динамику состояний и выходных сигналов.
	
	В итоге я закрепила теорию и получила практические навыки моделирования линейных систем управления.
	...
	\begin{lstlisting}[caption={Общие настройки графиков}]
ax = gca;
ax.LineWidth = 1.5;          
ax.FontSize = 11;             
...
\noindent
Рабочая директория с исходниками лабораторной доступна по ссылке: \href{https://github.com/decadeos/ITMO/tree/master/3_course/lsau/lab1}{GitHub — lsau/lab1}.
