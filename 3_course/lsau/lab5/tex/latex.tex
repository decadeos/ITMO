...
\begin{document}
	...
	\newpage
	\section{Общее условие}
	
	В рамках лабораторной работы необходимо исследовать реальные объекты,
	сопоставить их с типовыми динамическими звеньями и для каждого объекта:
	
	\begin{itemize}
		\item определить тип звена;
		\item записать передаточную функцию в стандартизированном виде и найти её параметры;
		\item получить аналитические выражения временных и частотных характеристик
		(переходная, весовая, АЧХ, ФЧХ, ЛАЧХ, ЛФЧХ);
		\item построить графики характеристик по моделированию и сравнить с теоретическими.
	\end{itemize}
	...
	Теперь приведу её к стандартному виду. Для этого разделю числитель и знаменатель на \(\dfrac{k_m k_e}{R}\):
	\[
	W(s)=
	...
	Подставлю численные значения параметров:
	\[
	A(\omega)
	...
	\textbf{Итог: } Полученная ПФ соответствует апериодическому звену первого порядка;
	аналитические и моделированные характеристики совпадают.
	...
	чтобы привести выражение к стандартизированному виду:
	\[
	W(s) =
	\frac{1}{k_e}\,
	\frac{1}{
	...
	\textbf{Итог:} Полученная ПФ соответствует колебательному звену;
	аналитические и моделированные характеристики совпадают.
	...
	\textbf{Итог:} Полученная ПФ соответствует идеально интегрирующему звену;
	аналитические и моделированные характеристики совпадают.
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}[scale=1.2, line width=1pt]
			...
			\draw[fill=blue!20] (2,0.3) rectangle (3.5,1.7);
	...
	\textbf{Итог:} Полученная ПФ соответствует консервативному звену;
	аналитические и моделированные характеристики совпадают.
	...
	\section{Регулятор на операционном усилителе}
		...
		\begin{tikzpicture}
			...
			\fill (canvas cs:x=0cm,y=3cm) circle (2pt);
			\draw (0,3) -- (1,3);
			
			% резистор 1
			\filldraw[fill=blue!20] (1,2.7) rectangle (3,3.3);
			\node at (2,3.6) {$R_1$};
			
			\draw (3,3) -- (4,3) -- (4, 4.5);
			\fill (canvas cs:x=4cm,y=3cm) circle (2pt);
			\draw (4, 4.5) -- (5, 4.5);
			
			% конденсатор
			\draw[line width=1.5pt]  (5, 4) -- (5, 5);   
			\draw[line width=1.5pt]  (5.3, 4) -- (5.3, 5); 
			\node at (5.15, 5.3) {$C$};
			
			\draw (5.3, 4.5) -- (6.3, 4.5);
			
			% резистор 2
			\filldraw[fill=blue!20] (6.3, 4.2) rectangle (8.3, 4.8);
			\node at (7.3, 5.1) {$R_2$};
			
			\draw (8.3, 4.5) -- (9.3, 4.5) -- (9.3, 2.5);
			\draw (4, 3) -- (6.3, 3);
			
			% Gain
			\filldraw[fill=blue!20] (6.3, 3.5) -- (6.3, 1.5) -- (8, 2.5) -- cycle;
			\node[text=white, font=\Large\boldmath] at (6.6, 3.0) {$-$};
			\node[text=white, font=\Large\boldmath] at (6.6, 2.0) {$+$};
			\draw (8, 2.5) -- (10, 2.5);
			\fill (canvas cs:x=10cm,y=2.5cm) circle (2pt);
			
			\draw (6.3, 2) -- (5.3, 2) -- (5.3, 1);
			\fill (canvas cs:x=5.3cm,y=1cm) circle (2pt);
			\draw (0, 1) -- (10, 1);
			\fill (canvas cs:x=0cm,y=1cm) circle (2pt);
			\fill (canvas cs:x=10cm,y=1cm) circle (2pt);
			
			\node at (0.5, 2) {$U_\text{вх}$};
			\node at (10, 1.8) {$U_\text{вых}$};
		\end{tikzpicture}
	...
	\[I_1 + I_f = 0 \quad \Longrightarrow \quad \frac{U_\text{вх}}{R_1} = - \frac{U_\text{вых}}{Z_f}	\]
	
	Запишу передаточную функцию:
	
	\[W(s) = \frac{U_\text{вых}}{U_\text{вх}} =  \frac{Z_f}{R_1} = \frac{R_2 + \dfrac{1}{s\,C}}{R_1}\]
	
	Приведу к стандартному виду:
	
	\[W(s) = \frac{R_2}{R_1} + \frac{1}{s} \cdot \frac{1}{R_1\,C} = \frac{R_2}{R_1} \left(1 + \frac{1}{R
	_2 C\,s}\right)\]
	
	Это передаточная функция ПИ-регулятора. Параметры:
	
	\[k_p = \frac{R_2}{R_1} \qquad k_i = \frac{1}{R_2 C}\]
	
	Подставлю параметры из моего варианта:
	
	\[k_p = \frac{16853}{1296} \qquad k_i = \frac{1}{16853 \cdot 419} \quad \Longrightarrow \quad k_p \approx 13 \qquad k_i = \frac{1}{16853 \cdot 419 }\]
	
	Тип звена ПИ-регулятора -- изодромное звено первого порядка.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task5/time/w5.png}
		\caption{Весовая функция}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task5/time/y5.png}
		\caption{Переходная функция}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task5/freq/ach5.png}
		\caption{АЧХ}
	\end{figure}
	...
	\[
	L(\omega) = 20\log_{10}\!\left(
	\sqrt{k_p^2 + \left(\frac{k_i}{\omega}\right)^2}
	\right)
	\]
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task5/freq/lach5.png}
		\caption{ЛАЧХ}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task5/freq/fch5.png}
		\caption{ФЧХ\protect\footnotemark[1]}
	\end{figure}
	
	ЛФЧХ представляет собой график зависимости фазовой характеристики 
	\(\varphi(\omega)\) в координатах \(( \log_{10}\omega , \varphi(\omega) )\).
	
		
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{../images/task5/freq/lfch5.png}
		\caption{ЛФЧХ\protect\footnotemark[1]}
	\end{figure}
	
	\textbf{Итог:} Полученная передаточная функция соответствует изодромному звену первого порядка;
	аналитические зависимости совпадают с результатами моделирования.

	\footnotetext[1]{Для корректного отображения фазовой характеристики при 
		\(\omega \rightarrow 0\) изменён диапазон параметров функции \texttt{logspace}.}
	...
	В работе я исследовала поведение пяти разных объектов, определила к каким
	типовым звеньям они относятся, нашла их передаточные функции и построила
	временные и частотные характеристики. Полученные формулы совпали с
	моделированием, значит всё рассчитано правильно.
